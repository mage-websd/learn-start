...
class SM_Xpermission_Block_Permissions_User_Edit_Tab_Main extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _prepareForm() {
        ...
        if (!Mage::app()->isSingleStoreMode()) {
            if (Mage::getSingleton('admin/session')->getUser()->isRoot()) {
                ...
                $fieldset->addField('default_store_id', 'hidden', array(
                    'name'      => 'default_store_id',
                    'value'     => Mage::getSingleton('admin/session')->getUser()->getDefaultStoreId()
                ));
            }
        }
        else {
        ...
        if (Mage::getSingleton('admin/session')->getUser()->isRoot()) {
            $fieldset->addField('is_root', 'select', array(
                    'name'      => 'is_root',
                    'label'     => Mage::helper('adminhtml')->__('Is Root'),
                    'title'     => Mage::helper('adminhtml')->__('Is Root'),
                    'required'  => true,
                    'options'	=> array(
            ...
            $model->setIsRoot(Mage::getSingleton('admin/session')->getUser()->getIsRoot());
            $fieldset->addField('is_root', 'hidden', array(
                    'name' => 'is_root',
                    'id'   => '_is_root',
            ));
        }
        ...
        $data = $model->getData();
		$data = $model->setData('default_store_id',Mage::getSingleton('admin/session')->getUser()->getDefaultStoreId());
        unset($data['password']);
