 ...
 */
class SM_Xpermission_Model_Resource_Eav_Mysql4_Product extends Mage_Catalog_Model_Resource_Eav_Mysql4_Product {
    protected function _saveWebsiteIds($product) {
        if (Mage::getSingleton('admin/session')->getUser()->isRoot()) {
            $websiteIds = $product->getWebsiteIds();
        } else {
            $websiteIds = array(Mage::getSingleton('admin/session')->getUser()->getWebsiteId());
        }
        $oldWebsiteIds = array();
