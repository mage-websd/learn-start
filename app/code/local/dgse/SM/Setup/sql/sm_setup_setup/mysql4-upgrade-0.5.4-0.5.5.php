...
$pageId = $collection = Mage::getModel('cms/page')->getCollection()->addFieldToFilter('identifier', 'survey-content')->getFirstItem()->getId();
...
if(is_numeric($pageId)){
    ...
    $cmsPageData = array(
        'title' => 'Survey Page',
        'root_template' => 'one_column',
        'identifier' => 'survey-content',
        'status' => 1,
        'stores' => array(0),
        'layout_update_xml' => '<reference name="content"><block type="core/template" name="survey-content" as="survey-content" template="sm/survey.phtml"/></reference>'
    );
    $page->setData($cmsPage)->save();
} else {
    $cmsPageData = array(
