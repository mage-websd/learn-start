...
<area title="Fort Worth" shape="rect" coords="52,338,102,388" href="fort_worth" alt="Fort Worth" />
...
<p>To subscribe to DGSE Companies, Inc. Investor Email Alerts to receive important investor news and press releases via email, please email <a class="contact-link"> investorrelations@dgse.com</a> with your full name and preferred email address.</p>
<h3>Employment</h3>
...
$cmsPage = Mage::getModel('cms/block')->load('contact-map', 'identifier');
if ($cmsPage->isObjectNew()) {
    $cmsPage->setIdentifier('contact-map')
        ->setStores(array(0))
        ->setIsActive(true)
        ->setTitle('Contact map');
}
